...
export async function updateProfile(req, res, next) {
  try {
    ...
    if (user.role === "pharmacist") {
      ...
      await updatePharmacist(pharmacistId, pharmacistData);
    }
...
export async function getSingleUsers(req, res, next) {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .send({ message: "Patient not found by the given id...!" });
    }
...
export async function getSingleDoctors(req, res, next) {
  try {
    ...
    if (!doctor) {
      return res
        .status(400)
        .send({ message: "Doctor not found by the given id" });
    }
...
export async function getAdminProfileByToken(req, res, next) {
  try {
    ...
    if (!admin) {
      return res
        .status(400)
        .send({ message: "admin not found by the given id" });
    }
...
export async function getSinglePharmacy(req, res, next) {
  try {
    ...
    if (!pharmacist) {
      return res
        .status(400)
        .send({ message: "pharmacist not found by the given id" });
    }
