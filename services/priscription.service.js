...
export async function updateByToken(doctorId, data) {
  ...
  const prescriptioData = await prescriptionModel.findByIdAndUpdate(
    prescriptionId,
    data,
    { new: true }
  );
...
export async function DeleteByToken(doctorId) {
  ...
  const prescriptionData = await prescriptionModel.findByIdAndDelete(
    prescriptionId
  );
...
export async function Delete(prescriptionId) {
  ...
  if (!prescriptionData)
    throw new HttpException(400, "No prescription is there");
