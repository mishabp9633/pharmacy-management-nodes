...
export async function getAll(doctorId) {
  const appoinmentData = await appoinmentModel
...
export async function update(data, appoinmentId) {
  const appoinmentData = await appoinmentModel.findByIdAndUpdate(
    appoinmentId,
    data,
    { new: true }
  );
  if (!appoinmentData)
    throw new HttpException(400, "appoinment Data is not found");
  return { appoinmentData };
}
